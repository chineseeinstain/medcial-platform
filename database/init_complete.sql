...
-- 插入科室运营数据（使用 INSERT IGNORE 避免重复键错误）
...
-- 插入医保结算数据（使用 INSERT IGNORE 避免重复键错误）
INSERT IGNORE INTO insurance_settlement (settlement_no, patient_id, settlement_date, total_cost, insurance_pay, personal_pay, insurance_type) VALUES
...
-- 插入药品库存数据（使用 INSERT IGNORE 避免重复键错误）
...
('DRUG011', '板蓝根颗粒', '中成药', '10g*20袋', '盒', 180, 60, 15.00, 22.00, '广州白云山', '2026-08-31', '正常'),
('DRUG012', '双氯芬酸钠缓释片', '解热镇痛', '75mg*10片', '盒', 130, 40, 20.00, 30.00, '诺华制药', '2026-09-30', '正常'),
('DRUG013', '氨溴索口服液', '止咳化痰', '15mg/5ml*100ml', '瓶', 110, 35, 18.00, 26.00, '勃林格殷格翰', '2026-06-30', '正常'),
('DRUG014', '阿司匹林肠溶片', '心血管', '100mg*30片', '盒', 200, 80, 6.00, 10.00, '拜耳医药', '2026-11-30', '正常'),
('DRUG015', '氯雷他定片', '抗过敏', '10mg*6片', '盒', 170, 50, 22.00, 32.00, '先灵葆雅', '2026-10-31', '正常'),
('DRUG016', '复方丹参片', '心血管', '0.32g*60片', '瓶', 95, 30, 12.00, 18.00, '北京同仁堂', '2026-07-31', '正常'),
('DRUG017', '感冒灵颗粒', '中成药', '10g*9袋', '盒', 220, 70, 16.00, 24.00, '三九药业', '2026-12-31', '正常'),
('DRUG018', '左氧氟沙星片', '抗生素', '0.5g*6片', '盒', 125, 40, 35.00, 52.00, '第一三共', '2026-08-31', '正常'),
('DRUG019', '多潘立酮片', '消化系统', '10mg*30片', '盒', 145, 45, 14.00, 20.00, '西安杨森', '2026-09-30', '正常'),
('DRUG020', '葡萄糖注射液', '注射液', '5%*250ml', '瓶', 500, 200, 3.50, 6.00, '科伦药业', '2026-06-30', '正常');

-- 插入人员信息（使用 INSERT IGNORE 避免重复键错误）
...
-- 注意：实际使用时需要通过Java代码生成BCrypt hash
-- 可以使用 PasswordGenerator.java 工具类生成
-- 临时使用一个BCrypt hash（密码：123456）
...
SELECT COUNT(*) AS '药品库存记录数' FROM drug_inventory;
SELECT COUNT(*) AS '人员信息记录数' FROM staff_info;
SELECT COUNT(*) AS '用户记录数' FROM user;
